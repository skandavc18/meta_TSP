...
cl_kernel max_fitness(cl_program program,cl_mem buffer,cl_mem elite_buffer)
{
...
cl_kernel min_fitness(cl_program program,cl_mem buffer,cl_mem least_elite_buffer)
{
...
cl_kernel rand_init(cl_program program,cl_mem buffer,int rand_num)
{
    ...
    err = clSetKernelArg(kernel, 0, sizeof(cl_mem), &buffer);
    err |= clSetKernelArg(kernel, 1, sizeof(cl_mem), &rand_num);
    if(err < 0) {
      perror("Couldn't create a kernel argument");
      exit(1);
    }
...
{
    cl_kernel kernel = clCreateKernel(program,"random_walk_of_all_ants", &err);
    if(err < 0) {
      perror("Couldn't create a kernel");
      exit(1);
    };
    ...
    err |= clSetKernelArg(kernel, 5, sizeof(cl_mem), &curr_iter);
    err |= clSetKernelArg(kernel, 6, sizeof(cl_mem), &rand_num);
    if(err < 0) {
      perror("Couldn't create a kernel argument");
      exit(1);
    }
...
{
    cl_kernel kernel = clCreateKernel(program,"mem_replace", &err);
    if(err < 0) {
      perror("Couldn't create a kernel");
      exit(1);
    };
...
int ALO(int *graph,int *ub)
{
    ...
    cl_int err;
    cl_mem antlion_buffer, ant_buffer,graph_buffer,elite_buffer,temp_elite_buffer,least_elite_buffer,ub_buffer;
    ...
    antlion_buffer = clCreateBuffer(context, CL_MEM_READ_WRITE |
         CL_MEM_COPY_HOST_PTR, SEARCH_AGENTS * sizeof(member),&antlions, &err); 
    ...
    kernel[4]=mem_copy(program,elite_buffer,antlion_buffer,temp_elite_buffer);
